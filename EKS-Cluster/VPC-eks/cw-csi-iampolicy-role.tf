data "aws_iam_policy_document" "cw_csi" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:amazon-cloudwatch:cloudwatch-agent"] #namespace and service account name must be this
    }
 /*       Condition = {
          StringEquals = {            
...
resource "aws_iam_role" "eks_cw_csi_driver" {
  assume_role_policy = data.aws_iam_policy_document.cw_csi.json
  name               = "${local.name}-cw-csi-iam-role"
}

resource "aws_iam_role_policy_attachment" "amazon_cw_csi_driver" {
  role       = aws_iam_role.eks_cw_csi_driver.name
